...
class FinancialRelease(models.Model):
    ...
    total_value = models.DecimalField(verbose_name=_("Total Value"), max_digits=settings.DEFAULT_MAX_DIGITS,
                                      decimal_places=settings.DEFAULT_DECIMAL_PLACES, help_text=_("If the type of payment is parceled out, this is the real total!"))
    type_of_payment = models.ForeignKey("config.TypeOfPayment", on_delete=models.SET_NULL, null=True,
    ...
    total_parcels = models.IntegerField(verbose_name=_("Total Parcels"), default=1)
    cost_center = models.ForeignKey("config.CostCenter", on_delete=models.SET_NULL, null=True, verbose_name=_("Cost Center"))
